...
def checkPasswordPolicy(min,max,letter,password):
    counter=0

    #print(f'max {max} min {min} pass {password}')
    for i in password:
        if i == letter:
            counter+=1

    # for char_index in range(len(password)):
    #     print(password[char_index])
...
def PasswordPolicyManager():
    ...
    for x in data:
        ...
        if checkPasswordPolicy(int(ranges[0]),int(ranges[1][:2]),policy[0][-1], str(policy[1:])):
            counter+=1
...
print(f"number that match: {PasswordPolicyManager()}")
